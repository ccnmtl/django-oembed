    ...
    if args:
        ...
        if not width and height:
            raise template.TemplateSyntaxError(
                "Oembed's optional WIDTHxHEIGH"
                "T argument requires WIDTH and HEIGHT to be positive "
                "integers.")
    else:
...
def do_oembed(parser, token):
    """
    A node which parses everything between its two nodes, and replaces
    any links with OEmbed-provided objects, if possible.

    Supports one optional argument, which is the maximum width and
    height, specified like so:
